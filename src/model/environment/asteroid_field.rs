...
impl Debris {
    pub fn new(pos: (f32, f32), size: f32) -> Self {
        Debris {
            pos,
            angle: random::f32() % std::f32::consts::TAU, // Random angle in range [0, 2*PI]
            speed: -(random::f32() * 0.2 + 0.1), // Negative angular speed for clockwise motion
            size: 6.0 + size/3. * random::f32(), // Random radius in range [960.0, 1280.0]
            lifetime: 30 + random::u32() % 30, // Random lifetime in range [60, 120]
            timer: 0,
        }
    }
...
impl Asteroid {
    pub fn new() -> Self {
        // Use the assumed `random::u32()` function to generate random values
        let angle = std::f32::consts::FRAC_PI_2 + 0.62; // Start at the top middle (90 degrees or π/2 radians)
        let speed = -(random::f32() * 0.0001 + 0.0001); // Negative angular speed for clockwise motion
        let radius = 1920.0 + 384.0 * random::f32(); // Random radius in range [960.0, 1280.0]
        let size = 8.0 + (random::f32() * 17.); // Random size in range [8.0, 24.0]
        let id = random::u32(); // Unique ID for the asteroid
...
pub static ASTEROID_FIELD: Lazy<Mutex<AsteroidField>> = Lazy::new(|| {
    Mutex::new(AsteroidField::new())
});
#[turbo::serialize]

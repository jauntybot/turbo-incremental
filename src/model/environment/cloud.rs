...
impl Cloud {
    pub fn new(center: (i32, i32), radius: u32, start: u32, color: u32) -> Self {
        ...
        while current_radius <= radius {
            ...
            for i in 0..num_circles {
                ...
                let color = if random::u16()%2==0 { 0x1a1229ff } else { 0x1f122bff };

                ring.push(Circle {
                    pos,
                    size: size_range.0 + (size_range.1 as f32 * random::f32()) as u32,
                    color,
                    angle,
                    speed: 0.1 + random::f32() * 0.04, // Random wobble speed
                    wobble_phase: random::f32() * std::f32::consts::TAU, // Random wobble phase
                    wobble_amplitude,
                });
            }
            rings.push(ring);
    ...
    pub fn update(&mut self) {
        for (ring_index, ring) in self.rings.iter_mut().enumerate() {
            for (circle_index, circle) in ring.iter_mut().enumerate() {
                ...
                let phase = time::tick() as f32 * speed + circle_index as f32 * 0.4 + ring_index as f32 * 0.8 + circle.angle * 2.0;
                circle.pos = (
    ...
    pub fn draw(&self) {
        for i in 0..self.rings.len() {
            for circle in self.rings[self.rings.len() - i - 1].iter() {
                ...
                let color = self.color | alpha as u32;

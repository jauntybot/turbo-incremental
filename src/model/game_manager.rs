...
pub struct GameManager {
    ...
    pub music_slider: Slider,
    pub sfx_toggle: Btn,
    pub reset_button: Btn,
    pub save_button: Btn,
    pub autosave: bool,
    pub autosave_toggle: Toggle,
}

// The singleton instance
pub static GLOBAL: Lazy<Mutex<GameManager>> = Lazy::new(|| Mutex::new(GameManager::new()));
impl GameManager {
    // Private constructor
    pub fn new() -> Self {
    ...
    pub fn update(&mut self, event_manager: &mut EventManager) {
        ...
        } else if p.just_pressed() && !p.intersects(self.menu_bounds.x(), self.menu_bounds.y(), self.menu_bounds.w(), self.menu_bounds.h()) && self.options {
            self.options = false;
        }
        self.info_button.update();
        if self.info_button.on_click() {
            self.info = !self.info
        } else if p.just_pressed() && !p.intersects(self.menu_bounds.x(), self.menu_bounds.y(), self.menu_bounds.w(), self.menu_bounds.h()) && self.info {
            self.info = false;
        }
        if self.options {
    ...
    pub fn draw(&self) {
        ...
        if self.options {
            ...
            text!(
                "MUSIC",
                fixed = true,
                xy = (self.music_slider.bounds.x() - 31, self.music_slider.bounds.center_y() - 3),
            );
        ...
        if self.info {
            ...
            let mut textbox = TextBox::new("alpha v0.2.3");
